Ext.define('pharmacy.view.productsController', {
    ...
    loadAllproducts: async function () {
        let productsList = this.lookupReference('grdAllProducts');
        let response = await Ext.Ajax.request({ url: '/products', method: 'get' });
        if (response.responseText) {
            let records = JSON.parse(response.responseText);
            let store = Ext.create('Ext.data.Store', { data: records });
            productsList.setStore(store);
            store.load();
        }
    }
